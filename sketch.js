...
function unitPlayer(sizeX, sizeY){
  ...
  this.drawBody = function(){
    if("right" == heading){
      ...
      this.drawAfterImage(x,y,-1*this.orientation,this.sizeX,this.sizeY);
    }
    push();
      this.checkBehavior();
      rectMode(CENTER);
  ...
  this.jump = function(){
    ...
    this.offsetY++;
    finalOffsetY = 0.4*(-1*((this.offsetY-sqrt(jumpHeight))*(this.offsetY-sqrt(jumpHeight))) + jumpHeight);
    if(finalOffsetY < 0){
      this.offsetY = 0;
      this.isJumping =  false;
      finalOffsetY = 0;
